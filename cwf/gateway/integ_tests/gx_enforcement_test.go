...
func TestGxMidSessionRuleRemovalWithCCA_U(t *testing.T) {
	...
	assert.Eventually(t, tr.WaitForEnforcementStatsForRule(imsi, "static-pass-all-2"), 10*time.Second, 2*time.Second)
	...
	assert.Eventually(t, tr.WaitForEnforcementStatsForRuleGreaterThan(imsi, "static-pass-all-2", 0), 10*time.Second, 2*time.Second)
